...
#define STK_LT_DEF     2010
#define STK_HT_DEF     2050
...
static int stk_ps_tune_zero_final(struct stk3x1x_priv *obj)
{
	...
	{
		obj->ps_high_thd_boot = obj->ps_stat_data[1] + 350;
		obj->ps_low_thd_boot = obj->ps_stat_data[1] + 300;	
		obj->boot_cali = 40;
	}
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(diff == obj->psi)
	{
		...
		obj->ps_high_thd_boot = obj->psi + 350;
		...
		if(atomic_read(&obj->ps_high_thd_val) > obj->ps_high_thd_tmp)
		{
			...
			if(obj->boot_cali >= 1 && obj->boot_ct != 0xFFFF)
			{
				obj->ps_high_thd_boot = obj->boot_ct + 350;
		...
		{
			obj->boot_ct = obj->psi;
			obj->ps_high_thd_tmp = obj->boot_ct + 350;
			obj->ps_low_thd_tmp = obj->boot_ct + 300;
...
#if defined(CONFIG_OF)
static irqreturn_t stk3308_eint_handler(int irq, void *desc)
{
	stk3x1x_eint_func();
...
//	stk3x1x_obj = obj;
	...
	if (stk3x1x_obj->irq_node)
	{
		...
		if(request_irq(stk3x1x_obj->irq, stk3308_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL)) {
    ...
    mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_POLARITY, stk3x1x_eint_func, 0);
    mt_eint_unmask(CUST_EINT_ALS_NUM);
#else
...
static int stk3x1x_init_client(struct i2c_client *client)
{
	...
	printk("stk3x1x_init_client  obj->first_boot =%d\n",obj->first_boot );
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	return sprintf(buf, "stk3308 Chip");  
}
		...
		case ALSPS_GET_ALS_RAW_DATA:    
			...
			break;
					...
					case ALSPS_IOCTL_PS_SET_CALI:
						...
						if(!g_ps_cali_flag)
						{
							g_ps_base_value = 1000; // set default base value
...
//	}
	if(0 != (err = stk3x1x_create_attr(&stk3x1x_init_info.platform_diver_addr->driver)))
	{
		APS_ERR("create attribute err = %d\n", err);
		goto exit_create_attr_failed;
	}
